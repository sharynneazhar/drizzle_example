var Collectible = artifacts.require('Collectible');
...
const main = async (cb) => {
  try {
    ...
    let txn1 = await collectible.mint('URI1');
    let tokenId1 = txn1.logs[2].args[0].toNumber();
    await marketplace.listNft(collectible.address, tokenId1, 1, {
      value: listingFee,
    });
    console.log(`Minted and listed ${tokenId1}`);
    let txn2 = await collectible.mint('URI1');
    let tokenId2 = txn2.logs[2].args[0].toNumber();
    await marketplace.listNft(collectible.address, tokenId2, 1, {
      value: listingFee,
    });
    console.log(`Minted and listed ${tokenId2}`);
    let txn3 = await collectible.mint('URI1');
    let tokenId3 = txn3.logs[2].args[0].toNumber();
    await marketplace.listNft(collectible.address, tokenId3, 1, {
      value: listingFee,
    });
    ...
    await marketplace.buyNft(collectible.address, tokenId1, { value: 1 });
    await marketplace.buyNft(collectible.address, tokenId2, { value: 1 });
    await logNftLists(marketplace);

    console.log('RESELL 1 NFT');
    await marketplace.resellNft(collectible.address, tokenId2, 1, {
      value: listingFee,
    });
    await logNftLists(marketplace);
  } catch (err) {
